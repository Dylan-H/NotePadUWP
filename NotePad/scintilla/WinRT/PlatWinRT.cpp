...
#define FONTS_CACHED
	...
	class SurfaceD2D : public Surface {
		...
		//CanvasActiveLayer clipsActive{nullptr};
		//std::vector<CanvasActiveLayer> clipsActive;
		Windows::Foundation::Collections::IVector<CanvasActiveLayer> clipsActive{ winrt::single_threaded_vector<CanvasActiveLayer>() };
		//winrt::array_view<CanvasActiveLayer> clipsActive;
	...
	void SurfaceD2D::Clear() {
		...
		if (pRenderTarget) {
			for (auto layer : clipsActive) {
				//IClosable iclosable = layer.try_as<IClosable>();
				//iclosable.Close();
				layer.Close();
				//CanvasActiveLayer al{nullptr};
				//winrt::attach_abi(al, layer);
				//al.Close();
			}
	...
	*/
	...
	XYPOSITION SurfaceD2D::WidthText(Font &font_, std::string_view text) {
		...
		if (pTextFormat) {
			...
			width = textLayout.LayoutBoundsIncludingTrailingWhitespace().Width;
		}
		return width;
	}
	...
	void SurfaceD2D::SetClip(PRectangle rc) {
		if (pRenderTarget) {
			...
			clipsActive.Append(pRenderTarget.CreateLayer(1, rcClip));
			//clipsActive.push_back(layer);
