...
SciTEBase::SciTEBase(Extension *ext) : apis(true), pwFocussed(&wEditor), extender(ext) {
	...
	propsPlatform.name = "propsPlatform";
	propsEmbed.name = "propsEmbed";
	propsBase.name = "propsBase";
	propsUser.name = "propsUser";
	propsDirectory.name = "propsDirectory";
	propsLocal.name = "propsLocal";;
	propsDiscovered.name = "propsDiscovered";
	props.name = "props";
	propsAbbrev.name = "propsAbbrev";
	propsSession.name = "propsSession";
	propsStatus.name = "propsStatus";
...
void SciTEBase::BraceMatch(bool editor) {
	...
	if ((braceAtCaret != -1) && (braceOpposite == -1)) {
		...
		if (props.GetInt("highlight.indentation.guides"))
			win.Call(SCI_SETHIGHLIGHTGUIDE, Minimum2(columnAtCaret, columnOpposite), 0);
	}
}
	...
	if (inSelection) {
		...
		if (selType == SC_SEL_LINES) {
			...
			for (int i=0; i<countSelections; i++) {
				startPosition = Minimum2(startPosition, wEditor.Call(SCI_GETSELECTIONNSTART, i));
				endPosition = Maximum2(endPosition, wEditor.Call(SCI_GETSELECTIONNEND, i));
			}
		}
		if (startPosition == endPosition) {
			return -2;
...
GUI::Rectangle SciTEBase::GetClientRectangle() {
	return GUI::Rectangle(); //wContent.GetClientPosition();
}
...
void SciTEBase::GoMatchingPreprocCond(int direction, bool select) {
	...
	int mppcMatch = -1;
	const int forward = true; //(direction == IDM_NEXTMATCHPPC);
...
void SciTEBase::EnsureRangeVisible(GUI::ScintillaWindow &win, int posStart, int posEnd, bool enforcePolicy) {
	const int lineStart = win.Call(SCI_LINEFROMPOSITION, Minimum2(posStart, posEnd));
	const int lineEnd = win.Call(SCI_LINEFROMPOSITION, Maximum2(posStart, posEnd));
...
//void SciTEBase::ContextMenu(GUI::ScintillaWindow &wSource, GUI::Point pt, GUI::Window wCmd) {
	...
	//const char *endDefinition = userContextItem + 0;//userContextMenu.length();
