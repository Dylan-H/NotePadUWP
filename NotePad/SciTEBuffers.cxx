...
bool Buffer::NeedsSave(int delayBeforeSave) const {
	...
	return now && documentModTime && isDirty &&  (now-documentModTime > delayBeforeSave) && !file.IsUntitled() && !failedSave;
}
...
void SciTEBase::Close(bool updateUI, bool loadingSession, bool makingRoomForNew) {
	...
	if (buffers.size() == 1) {
		...
		closingLast = (buffers.lengthVisible == 1);
		if (closingLast) {
			buffers.buffers[0].Init();
			...
			//if (buffers.buffers[buffers.Current()].file.) {
