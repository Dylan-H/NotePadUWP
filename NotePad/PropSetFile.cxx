...
static bool GenericPropertiesFile(const FilePath  &filename) {
	std::string name = GUI::UTF8FromString(filename.BaseName());
...
PropSetFile::ReadLineState PropSetFile::ReadLine(const char *lineBuffer, ReadLineState rls, const FilePath &directoryForImports,
	...
	if (isprefix(lineBuffer, "if ")) {
		...
		//if (directoryForImports.Name().size()>0) {
...
winrt::Windows::Foundation::IAsyncOperation<bool> PropSetFile::ReadAsync(const FilePath &filename, const FilePath &directoryForImports, const ImportFilter &filter,
	FilePathSet *imports, size_t depth) {
	auto hstring = co_await filename.ReadAsync();
	std::string propsData = winrt::to_string(hstring);
...
bool PropSetFile::Read(const FilePath &filename, const FilePath &directoryForImports,
                       const ImportFilter &filter, FilePathSet *imports, size_t depth) {
	std::string propsData = filename.Read();
...
bool IsPropertiesFile(const FilePath &filename) {
	...
	if (EqualCaseInsensitive(GUI::UTF8FromString(ext).c_str(), PROPERTIES_EXTENSION + 1))
		return true;
	return false;
}
