...
winrt::Windows::Foundation::IAsyncAction SciTEBase::ReadEmbeddedProperties() {
	return nullptr;
}
...
*/
winrt::Windows::Foundation::IAsyncAction SciTEBase::ReadGlobalPropFile() {
	std::string excludes;
	std::string includes;
	...
	for (int attempt=0; attempt<1; attempt++) {
	    ...
	    co_await ReadEmbeddedProperties();

		propsBase.Clear();
		FilePath propfileBase = GetDefaultPropertiesFileName().get();
		co_await propsBase.ReadAsync(propfileBase, propfileBase, filter, &importFiles, 0);

		propsUser.Clear();
		///FilePath propfileUser = GetUserPropertiesFileName();
		//propsUser.Read(propfileUser, propfileUser, filter, &importFiles, 0);
	}
...
void SciTEBase::ReadAbbrevPropFile() {
	propsAbbrev.Clear();
	propsAbbrev.Read(pathAbbreviations, pathAbbreviations, filter, &importFiles, 0);
}
...
void SciTEBase::ReadDirectoryPropFile() {
	...
	if (props.GetInt("properties.directory.enable") != 0) {
		...
		props.Set("SciteDirectoryHome", propfile.AsUTF8().c_str());

		propsDirectory.Read(propfile, propfile, filter, nullptr, 0);
	}
}
...
void SciTEBase::ReadLocalPropFile() {
	...
	//propsLocal.Clear();
	//propsLocal.Read(propfile, propfile, filter, nullptr, 0);
...
std::string SciTEBase::ExtensionFileName() const {
	if (CurrentBufferConst()->overrideExtension.length()) {
		...
		if (name.IsSet()) {
			...
			std::string extension = GUI::UTF8FromString(name.Extension());
			if (extension.empty()) {
				return name.AsUTF8();
			} else {
				LowerCaseAZ(extension);
				return  GUI::UTF8FromString(name.BaseName()) + extension;
			}
...
concurrency::task <FilePath> SciTEBase::GetDefaultPropertiesFileName() {
	//auto uri =  winrt::Windows::Foundation::Uri(L"ms-appx:///samples/logo.png");
	return concurrency::create_task([]
	{
		std::wstring fname = L"ms-appx:///Assets/";
		fname += propGlobalFileName;
		auto uri = winrt::Windows::Foundation::Uri(fname);
...
FilePath SciTEBase::GetAbbrevPropertiesFileName() {
	auto uri = winrt::Windows::Foundation::Uri(propAbbrevFileName);
	auto file = winrt::Windows::Storage::StorageFile::GetFileFromApplicationUriAsync(uri).get();
	return FilePath(file);
}

FilePath SciTEBase::GetUserPropertiesFileName() {
	//auto uri = winrt::Windows::Foundation::Uri(propUserFileName);

	std::wstring fname = L"ms-appx:///Assets/";
	fname += propUserFileName;
	auto uri = winrt::Windows::Foundation::Uri(fname);
...
FilePath SciTEBase::GetLocalPropertiesFileName() {
	auto uri = winrt::Windows::Foundation::Uri(propLocalFileName);
	auto file = winrt::Windows::Storage::StorageFile::GetFileFromApplicationUriAsync(uri).get();
	return FilePath(file);
}
...
FilePath SciTEBase::GetDirectoryPropertiesFileName() {
	...
	//if (filePath.IsSet()) {
	//	propfile.Set(filePath, propDirectoryFileName);

	//	// if this file does not exist try to find the prop file in a parent directory
	//	while (!propfile.IsRoot() && !propfile.Exists()) {
	//		propfile.Set(propfile, propDirectoryFileName);
	//	}
